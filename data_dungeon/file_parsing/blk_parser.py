...
def parse_transaction(mmap_obj, pos):
    ...
    transaction['Inputs Count'] = inCount
    ...
    tmpHex = tmpHex + tmpB
    pos += offset
    transaction['Outputs Count'] = outputCount
    RawTX += reverse(tmpHex)
...
def parse_block(mmap_obj, start_pos=0):
    ...
    block_size = read_bytes_from_str(mmap_obj, pos, 4)
    pos += 4
...
def transaction_parsing(file_path, start_pos=0):
